...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderCreateDto create(OrderCreateDto orderCreateDto) throws Exception {
       ...
       // if (Objects.equals(userDetails.loadUserByUsername(), user.getUsername())){
...
//
    ...
    public void updateForAdmin(OrderUpdateForAdmin orderUpdateForAdmin) {
        int priceOfBooks = 0;
        val books = bookRepo.findAllByIdIn(orderUpdateForAdmin.getOrderedBookIds());
        val user = userRepo.findById(orderUpdateForAdmin.getUserId()).orElseThrow();
        Order order = orderUpdateForAdmin.convertOrderUpdateForAdminToEntity(books, user);
        ...
        if (!order.getUser().getIsBlocked()) {
            ...
            if (priceOfBooks <= 10000) {
                ...
                throw new Exception("You have reached the purchase limit of 10000 ");
        } else throw new Exception("The user " + user.getUsername() + "is blocked");
    }
